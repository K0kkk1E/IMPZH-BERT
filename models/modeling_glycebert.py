...
class ImpGlyceBertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
    ...
    def forward(
        ...
        sequence_output = outputs[0]  # 使用Bert模型的序列输出
        pooled_output = outputs[1]   # [CLS]标记的输出

        pooled_output = self.dropout(pooled_output)

        lstm_output, (hidden_last,_) = self.bilstm(sequence_output,output_hidden_states)
        ...
        lstm_output = self.dropout(hidden_last_out)

        combined_output = pooled_output + lstm_output
